...
The upper left pane is the data viewer, which allows you to view and add or edit data. The lower left pane is the console, which allows you to see installation or errors and you can ask for help on it. The upper right pane is the environment, which allows you to see hidden data and you can commit and push your changes on it. The lower right pane is called the viewer which allows you to view a preview of the document.
...
-   Have a quick look at the data and notice that there are cells taking the value `NA` -- what does this mean? `NA` stands for non-applicable, and it means that there is no data for a variable.
...
All the continents have at least one peak of the amount of plastic waste to the left, and the data is skewed to the right. The peak for Europe is bigger than the other peaks for the other continents, and the peak for South America is the smallest peak, meaning South America has less plastic waste per capita. North America's data has an outlier where the plastic waste per capita is 3.6.
...
```
```{r plastic_waste_per_cap-hist-facet, eval=TRUE}
ggplot(data = plastic_waste, aes(x = plastic_waste_per_cap)) +
  geom_histogram(binwidth = 0.2)+
...
The `color` and `fill` are based on the data so they go in aesthetics, but the `alpha` level is not based on the data so it is plotted in geom.
...
The violin plots reveal the continents where plastic waste per capita is greater by their bulks. The box plots, however, features the median and quadrants for plastic waste per capita, and the violin plots don't show the medians.
...
1.  Visualize the relationship between plastic waste per capita and mismanaged plastic waste per capita using a scatterplot.
    ...
    As the plastic waste per capita increases, the mismanaged plastic waste increases but the rate slows down, for mismanaged plastic waste per capita skyrockets when there is less plastic waste per capita, but the rate decreases when there is more plastic waste per capita. The relationship is not linear.
```{r plastic_waste_per_cap-point, eval=TRUE}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
...
2.  Colour the points in the scatterplot by continent.
    ...
    There are clear distinctions, for it looks like the plastic waste per capita and the mismanaged plastic waste per capita is higher in Asia and lower in Europe, and there is an outlier in North America.
```{r plastic_waste_per_cap-point-color, eval=TRUE}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
                     y = mismanaged_plastic_waste_per_cap,
                     colour = continent)) +
...
3.  Visualize the relationship between plastic waste per capita and total population as well as plastic waste per capita and coastal population.
    ...
    They do not appear to be more linearly associated.
```{r plastic_waste_per_cap-point-one, eval=TRUE}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
                     y = total_pop)) +
  geom_point()
```
```{r plastic_waste_per_cap-point-two, eval=TRUE}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
                     y = coastal_pop)) +
  geom_point()
...
The data for the total population chart is pushed back as in less plastic waste per capita, unlike the coastal population proportion chart, where the data is more scattered with a wider range of plastic waste per capita.
...
plastic_waste %>% 
  ...
  ggplot(aes(x = total_pop, y = plastic_waste_per_cap, color = continent)) + 
    ...
    labs(x = "Total Population", 
         y = "Plastic waste per capita ", 
         color = "Continent",
         title = "Plastic waste vs. total population",
         subtitle = "by continent") +
    theme_minimal()
```
